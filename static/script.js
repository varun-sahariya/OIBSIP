class VoiceAssistantPro {
    ...
    initializeSocket() {
        ...
        this.socket.on('llm_complete', () => this.hideTyping());
    ...
    async startRecording() {
        ...
        try {
            ...
            const workletBlob = new Blob([`class PCMProcessor extends AudioWorkletProcessor { process(inputs) { this.port.postMessage(inputs[0][0]); return true; } } registerProcessor('pcm-processor', PCMProcessor);`], { type: 'application/javascript' });
            ...
            this.workletNode.port.onmessage = (event) => {
                ...
                if (audioBuffer.length >= 4096) {
                    ...
                    for (let i = 0; i < audioBuffer.length; i++) pcm16Data[i] = Math.max(-1, Math.min(1, audioBuffer[i])) * 0x7FFF;
    ...
    changePersona(e) {
        ...
        const personas = { default: "Hello! I'm your AI assistant, ready to help you with questions, tasks, and conversations in a professional and friendly manner.", pirate: "Ahoy matey! I be a salty pirate captain ready for adventure on the high seas! 🏴‍☠️", scientist: "Fascinating! I'm a brilliant scientist eager to explore the mysteries of the universe through experimentation! ⚗️", wizard: "By my ancient wisdom, I am a mystical wizard versed in the arcane arts and eternal knowledge! ✨", robot: "GREETINGS HUMAN. I AM A LOGICAL ROBOT UNIT DESIGNED TO PROVIDE OPTIMAL ASSISTANCE AND EFFICIENCY. BEEP BOOP! 🤖", chef: "Bonjour! I am a passionate master chef who lives and breathes the culinary arts! Magnifique! 👨‍🍳", detective: "Good day. I'm a sharp-eyed detective who notices every detail and solves mysteries with keen observation. 🔍" };
        this.personaPreview.textContent = personas[this.currentPersona];
        if (this.socket) this.socket.emit('persona_change', { persona: this.currentPersona });
    }
    ...
    clearChat() {
        this.chatHistory.innerHTML = `<div class="message assistant"><div class="message-avatar">🤖</div><div class="message-content">Welcome back! I'm ready to help you with anything you need.</div></div>`;
    }
    ...
    async processAudioQueue() {
        ...
        try {
            ...
            if (pcmData && pcmData.byteLength > 0) await this.processAudioChunk(pcmData);
        } catch (error) {
            console.error('Error processing audio queue:', error);
        } finally {
            this.isProcessingQueue = false;
            if (this.audioQueue.length > 0) setTimeout(() => this.processAudioQueue(), 10);
        }
    }

    async initializePlaybackContext() {
        if (!this.playbackContext || this.playbackContext.state === 'closed') {
            this.playbackContext = new (window.AudioContext || window.webkitAudioContext)({ sampleRate: 44100 });
    ...
    async processAudioChunk(pcmData) {
        try {
            ...
            for (let i = 0; i < samples; i++) channelData[i] = pcm16[i] / 32768;
        ...
        } catch (error) { console.error('Error processing audio chunk:', error); }
    }

    stopAudio() {
        if (this.playbackContext && this.playbackContext.state !== 'closed') {
            this.activeSourceNodes.forEach(source => { try { if (source.buffer) source.stop(); } catch (e) {} });
